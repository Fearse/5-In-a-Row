...
`timescale 1ns / 1ps
 module vga(
  ...
  clkNew<=~clkNew;
   ...
   wire score_gfx;  
   wire [2:0]colors;
   figure_generator figure_gen(.clk(clkNew),.vpos(vpos),.hpos(hpos),.board_gfx(score_gfx),.colors(colors),.ArrPos(ArrPos),.i(i),.j(j),.player(player),.reset(reset),.finish(finish));
